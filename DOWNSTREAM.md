...
- Web SSH provides a searchable command palette (`Ctrl+Shift+P`) for server/session operations, target switching/connection, terminal font size, panel visibility, connection instructions, disconnect actions, and theme changes. The palette supports arrow-key navigation, `Enter` to run, and `Esc` to close.
- Direct keyboard shortcuts cover common workspace actions without changing backend permissions: `Alt+1`…`Alt+9` switch connected server tabs, `Alt+A` opens the server picker, `Alt+N` opens a shell, `Alt+R` reconnects the active server, `Alt+B` toggles the server panel, and `Alt+M` toggles the metrics rail.
- The bottom of the metrics rail contains a discoverable Command Palette / keyboard-shortcuts hint, while the gear menu provides the same entry when the metrics rail is hidden. Rail metric cards use a denser vertical layout to reserve room for the help affordance without removing CPU/MEM/NET/LOAD/DISK/UP data.
